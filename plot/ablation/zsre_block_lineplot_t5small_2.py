...
base_dir = '../logs/paper'
settings = ['t5small_zsre_r2e75b0', 't5small_zsre_r2e75b2', 't5small_zsre_r2e75b4']
...
fig, axs = plt.subplots(1, 2, figsize=(5,2.5),dpi=800)
for i in range(2):
    for x in axs:
...
axs[1].plot(t, UP[0], label = '$VecDB = [0]$',linewidth=1.8,color=colors[0])
axs[1].plot(t, UP[1], label = '$VecDB = [2]$',linewidth=1.8,color=colors[1])
axs[1].plot(t, UP[2], label = '$VecDB = [4]$',linewidth=1.8,color=colors[2])
...
axs[1].set_ylabel('Locality',fontweight='bold',family = 'serif')
axs[1].grid(True)
...
plt.savefig('ablation_res/T5Small_zsre_block_2.jpg')
